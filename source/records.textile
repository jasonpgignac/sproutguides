...
NOTE: The model layer is also responsible for fetching data from and committing changes to your server when necessary. The server communication aspect of the model layer is not covered in this guide, but in the guide about using data sources.
...
We have used the +SC.Record.attr+ helper to add the +firstName+, +lastName+ and +age+ attributes with the type of the attribute as first argument. +SC.Record.attr+ also accepts an option hash as an optional second argument. This has can, for example, allow us to add some default values to our attributes:
