...
NOTE: In most cases you will find you can just ignore the +keyName+ parameter. Also, when you retrieve +keyname+ via +get+, +value+ will be +undefined+.
...
To work with relative property path you can use the +getPath+ and +setPath+ functions. These work identically to +get+ and +set+ except that they expect a path for the first argument.
